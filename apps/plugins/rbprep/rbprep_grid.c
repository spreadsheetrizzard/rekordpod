                              ...
                              uint32_t begin, uint32_t end)
{
    ...
    if (reader->fd < 0 || begin >= end || end > reader->beat_count)
        return false;
    cursor = begin;
    while (cursor < end) {
        struct rbprep_grid_page *page = cached_page(reader, cursor);
        uint32_t page_end;
...
bool rbprep_grid_cache_all(struct rbprep_grid_reader *reader)
{
    ...
    if (reader->fd < 0 || reader->page_count <= 0)
        return false;
    records_per_page = reader->page_bytes / RBPREP_GRID_RECORD_BYTES;
    capacity = records_per_page * reader->page_count;
    if (!records_per_page || reader->beat_count > capacity)
        return false;
    ...
    return rbprep_grid_range_cached(reader, 0, reader->beat_count);
}
