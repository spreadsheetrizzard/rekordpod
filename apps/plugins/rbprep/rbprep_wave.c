...
static struct rbprep_wave_page *cached_page(
                              ...
                              uint32_t begin, uint32_t end)
{
    ...
    if (reader->fd < 0 || begin >= end || end > reader->point_count)
        return false;
    cursor = begin;
    while (cursor < end) {
        struct rbprep_wave_page *page = cached_page(reader, cursor);
        uint32_t page_end;
                             ...
                             uint32_t begin, uint32_t end)
{
    ...
    while (cursor < end) {
        ...
        if (!page) {
            struct rbprep_wave_page *victim = find_page(reader, cursor);

            return victim && load_page(reader, victim, cursor);
        }
